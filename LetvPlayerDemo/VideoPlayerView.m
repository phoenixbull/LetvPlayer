...
- (UIButton *)replayBtn {
    if (!_replayBtn) {
        _replayBtn = [[UIButton alloc] initWithFrame:CGRectMake((_videoWidth - 100)/2 + kLrMargin, (_videoHeight - kUIHeight)/2 + kTopMargin, 100, kUIHeight)];
        _replayBtn.backgroundColor  =[UIColor blackColor];
        _replayBtn.titleLabel.adjustsFontSizeToFitWidth = YES;
        _replayBtn.hidden = YES;
        [_replayBtn setTitle:@"重新播放" forState:UIControlStateNormal];
        [_replayBtn addTarget:self action:@selector(replay) forControlEvents:UIControlEventTouchUpInside];
    }
    return _replayBtn;
}
...
- (void)addObservers{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playerItemNotification:) name:AVPlayerItemDidPlayToEndTimeNotification object:nil];
}
...
- (void)playerItemNotification:(NSNotification *)noti
{
